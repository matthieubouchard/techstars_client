...
export const Label = styled.label.attrs({ className: 'block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2' })``
export const buttonStyles = 'mt-5 mr-4 bg-white hover:bg-gray-100 text-gray-800 font-semibold py-2 px-4 border border-gray-400 rounded shadow cursor-pointer'
...
function AddCompany({ editing = false, company, closeAction }: AddCompanyProps) {
  ...
  return (
    <>
      <div className="w-full text-2xl text-center my-6 text-gray-700 font-semibold">{editing ? `Editing: ${company!.name}` : 'Add Company'}</div>
      <form className="w-10/12 mx-auto" onSubmit={handleSubmit(onSubmit)}>
        <div className="flex flex-wrap -mx-3 mb-6">
          ...
          <div className="w-full px-3 my-5">
            ...
            <textarea name="description" ref={register({ required: true })} className="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500 h-40" id="grid-last-name" placeholder="Description" />
